...
모드 / 설정파일 / 배경화면을 입력하고 "배포" 버튼 한 번으로 자동
업로드되게 해주는 정적 웹 도구입니다. (화이트리스트는 여기서 다루지 않습니다 — 아래
"화이트리스트 편집 권한 위임" 참고.) 백엔드 서버나 데이터베이스가 없고, 전부
...
- 서버 메타데이터(이름/설명/아이콘/주소/버전), Java 요구 버전 자동 계산,
...
## 화이트리스트 편집 권한 위임 (화이트리스트 키)

`index.html`의 "1-2. 화이트리스트 키 관리"에서 서버를 하나 이상 선택해 키를 발급하면,
그 키를 받은 사람은 `whitelist.html`을 열어 그 키만 입력하는 것으로 로그인해 선택된
서버의 화이트리스트만 편집할 수 있습니다. `UPLOAD_SECRET`을 알려줄 필요가 없고, 그
키로는 화이트리스트 외에는(모드/설정/다른 서버 등) 아무 것도 바꿀 수 없습니다.

- 키는 생성 직후 한 번만 화면에 표시됩니다 — 다시 조회할 수 없으니 그 자리에서 복사해서
  전달하세요. 잃어버리면 새로 발급하면 됩니다.
- "발급된 키 목록"에서 언제든 "삭제"로 즉시 폐기할 수 있습니다. 폐기 후에는 그 키로
  `whitelist.html`에 로그인할 수 없습니다.
- 키 하나에 여러 서버를 함께 묶어서 발급할 수 있습니다(발급 시 체크박스로 여러 개 선택).
- 자세한 API 동작은 `tools/distro-worker/README.md`를 참고하세요.
- `index.html`에는 화이트리스트 입력칸이 없습니다 — 여기서 "배포"를 눌러도 기존 서버의
  화이트리스트는 절대 건드리지 않습니다(빈 값으로 덮어써서 위임 키로 설정해둔 화이트리스트가
  날아가는 걸 막기 위해 의도적으로 분리했습니다).
...
1. "2. 서버 선택"에서 기존 서버를 고르면 폼이 자동으로 채워지고, "새 서버 만들기"를 고르면
   빈 폼으로 시작합니다.
2. 서버 정보, 배경화면(선택), 모드/설정 파일을 채웁니다. (화이트리스트는 위
   "화이트리스트 편집 권한 위임" 절차로 `whitelist.html`에서 따로 편집합니다.)
3. Java 요구 버전은 마인크래프트 버전을 입력하면 자동으로 채워집니다. 특수한 경우에만
   "수동으로 지정"을 체크하세요.
4. 기존 서버를 수정 중이라면 "8. 기존 모듈" 목록에서 삭제하고 싶은 모드/설정 파일만 체크하세요.
...
- **파일 크기**: 80MB가 넘는 파일은 자동으로 멀티파트 업로드(40MB 조각으로 나눠 순차
  전송)로 전환되어, GitHub API를 쓰던 시절의 25~30MB 한도와 달리 2GB 이상도 문제없이
  올라갑니다(150MB로 실측 확인, 조각 개수만 늘어나는 구조라 그보다 커도 동일하게
  동작). 2GB 넘는 파일을 추가하면 "시간이 오래 걸릴 수 있다"는 정보성 안내만 표시되고
  업로드 자체는 막히지 않습니다.
