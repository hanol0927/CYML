...
5. "GitHub에 배포"를 누르면: 새로 추가한 파일들을 자산 저장소에 커밋 → `distribution.json`을
   ...
   이때 기존 모듈 중 NeoNebula BASE_URL 슬래시 버그로 자산 저장소 이름이 빠진 채
   `<도메인>/repo/...`로 잘못 저장된 URL이 있으면 `<도메인>/<이 서버의 자산 저장소>/repo/...`로
   **자동으로 고쳐서** 함께 배포됩니다(별도 조작 불필요). 단, 이 자동 보정은 URL 문자열만
   고칠 뿐이라 그 경로에 실제 파일이 이미 올라가 있어야 다운로드가 됩니다 — NeoNebula 로컬
   실행 결과물(ROOT의 `repo` 폴더)을 그 서버의 자산 저장소에 직접 올려둔 적이 없다면 그
   폴더도 함께 올려야 합니다.
